...
from app.api import predict, train, applications
...
async def partial_result():
    with open("templates/partials/result.html", encoding="utf-8") as f:
        return HTMLResponse(f.read())


@app.get("/partials/history", response_class=HTMLResponse, include_in_schema=False)
async def partial_history():
    with open("templates/partials/history.html", encoding="utf-8") as f:
        return HTMLResponse(f.read())